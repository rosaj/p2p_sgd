...
def calc_agent_timeline(data, x_axis, agg_fn, metric="test_model-accuracy_no_oov"):
    ...
    for rind in rounds:
        test = [
            data[a_id][metric][rind]
            for a_id in data.keys()
            if metric in data[a_id]
            and (dataset_name is None or dataset_name == data[a_id]["dataset_name"])
        ]
        ...
        if x_axis == "epoch":
            ...
            comms = sum(
                [
                    sum(
                        data[a_key]["useful_msg"][:rind]
                        + data[a_key]["useless_msg"][:rind]
                    )
...
def resolve_timeline(
    filename, x_axis, agg_fn=np.average, metric="test_model-accuracy_no_oov"
):
    ...
    if isinstance(filename, list):
        ...
        for fl in filename:
            ...
            if acc_t is None:
                ...
                num_elem_acc = min(len(acc_t), len(acc))
                num_elem_time = min(len(time_t), len(time))
                acc_t = np.add(acc_t[:num_elem_acc], acc[:num_elem_acc])
                time_t = np.add(time_t[:num_elem_time], time[:num_elem_time])

        return (
            np.array(time_t) / float(len(filename)),
            np.array(acc_t) / float(len(filename)),
            accs,
        )
...
def plot_items(
    ...
    for i, (k, v) in enumerate(viz_dict.items()):
        x_time, t_acc, accs = parse_timeline(
            k, v, x_axis, agg_fn, metric[i] if isinstance(metric, list) else metric
        )
        ...
        if x_axis2 is not None:
            x_time2, t_acc2, accs2 = parse_timeline(
                k, v, x_axis2, agg_fn, metric[i] if isinstance(metric, list) else metric
            )
...
def side_by_side(
    viz_dict, agg_fn=np.average, fig_size=(10, 5), n_rows=1, axis_lim=None
):
    ...
    for ax, (plot_k, plot_v) in zip(axs, viz_dict.items()):
        ...
        plot_items(
            ...
            plot_v["viz"],
            title=plot_k,
            y_label=plot_v.get("y_label", "Test UA (%)"),
            colors=plot_v.get("colors", None),
            agg_fn=agg_fn,
            metric=metrics,
        )
    max_y = round(max([ax.get_ylim()[1] for ax in axs]))
