...
def load_clients_data(num_clients=100, seed=608361, train_examples_range=(700, 20_000), tags=['python', 'javascript'], is_bert=False):
    ...
    for tag, n_cli in zip(tags, num_clients):
        if not is_bert:
            ...
            train, val, test, metadata = load_bert_json_client_datasets(num_clients=num_clients, train_examples_range=train_examples_range, seed=-1, directory='bert_clients_tag/{}'.format(tag))
